...
provided, this will default to the bundle name. By default, Qliksense will 
...
file system. 
When you wish to install, upgrade or delete a bundle, Qliksense will use the 
credential set to determine where to read the necessary information from and
will then provide it to the bundle in the correct location. `,
		Example: `  qliksense credential generate
  qliksense bundle credential generate kubecred --insecure
  qliksense bundle credential generate kubecred --file myapp/porter.yaml
  qliksense bundle credential generate kubecred --tag deislabs/cool-bundle:v1.0.1
  qliksense bundle credential generate kubecred --cnab-file myapp/bundle.json --dry-run
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func buildCredentialsListCommand(p *porter.Porter) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  qliksense credentials list [-o table|json|yaml]`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func buildCredentialsShowCommand(p *porter.Porter) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  qliksense credential show NAME [-o table|json|yaml]`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
