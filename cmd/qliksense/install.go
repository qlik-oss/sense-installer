...
func installCmd(q *qliksense.Qliksense) *cobra.Command {
	...
	f.StringVarP(&opts.RotateKeys, "rotateKeys", "m", "", "Rotate JWT keys for qliksense (yes:rotate keys/ no:use exising keys from cluster/ None: use default EJSON_KEY from env")
	return c
}
